...
def get_all(table_name, limit=500):
    ...
    try:
        res = session.execute(text(f"SELECT * FROM `{table_name}` LIMIT :lim"), {"lim": limit})
        cols = res.keys()
        rows = res.fetchall()
...
def get_by_id(table_name, id_col, id_val):
    ...
    session = SessionLocal()
    try:
        res = session.execute(text(f"SELECT * FROM `{table_name}` WHERE `{id_col}` = :id_val LIMIT 1"), {"id_val": id_val})
        row = res.fetchone()
        if not row:
            return None
