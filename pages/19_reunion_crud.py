...
st.markdown("---")
st.subheader("Registrar reunión")
with st.form("create_reunion"):
    inputs = {}
    for c in cols:
        if c == ID_COL:
            continue
        if "fecha" in c.lower() or "dia" in c.lower():
    ...
    if st.form_submit_button("Registrar"):
        ...
        try:
            ...
            st.success(f"Reunión registrada (id: {new})")
        except Exception as e:
            st.error(e)

st.markdown("---")
st.subheader("Buscar / Editar / Eliminar")
buscar = st.text_input("Id reunion", "")
